 ...
 * @return array
 */
function progression_studios_body_classes( $classes ) {

	if ( get_theme_mod( 'progression_studios_header_width') == 'progression-studios-header-sidebar') {
		$classes[] = 'progression-studios-header-sidebar-before';
	}

	return $classes;
}
add_filter( 'body_class', 'progression_studios_body_classes' );
...
?>
	class="
		...
		<?php if ( get_theme_mod( 'progression_studios_nav_search') == 'off') : ?> progression-studios-search-icon-off<?php endif; ?>
...
?>
	...
	<?php if (get_theme_mod( 'progression_studios_header_sticky', 'sticky-pro' ) == 'sticky-pro' && get_theme_mod( 'progression_studios_logo_position' ) == 'progression-studios-logo-position-center' && get_theme_mod( 'progression_studios_header_width' ) != 'progression-studios-header-sidebar'  ) : ?><div id="progression-sticky-header"><?php endif; ?>
	...
	<?php if (get_theme_mod( 'progression_studios_header_sticky', 'sticky-pro' ) == 'sticky-pro' && get_theme_mod( 'progression_studios_logo_position' ) == 'progression-studios-logo-position-center' && get_theme_mod( 'progression_studios_header_width' ) != 'progression-studios-header-sidebar'  ) : ?></div><?php endif; ?>
...
function progression_studios_portfolio_link() {
	global $post;
?>
		...
		<?php if(get_post_meta($post->ID, 'progression_studios_blog_featured_image_link', true) == 'progression_link_url_new_window'): ?>
			...
			<?php else: ?>
				...
				<?php if(get_post_meta($post->ID, 'progression_studios_blog_featured_image_link', true) == 'progression_link_lightbox'): ?>
					...
					href="<?php echo esc_attr($image[0]);?>" data-rel="prettyPhoto[gallery-progression]" <?php $get_description = get_post(get_post_thumbnail_id())->post_excerpt; if(!empty($get_description)){ echo 'title="' . $get_description . '"'; } ?>
					
						<?php else: ?>
							
						<?php if(get_post_meta($post->ID, 'progression_studios_blog_featured_image_link', true) == 'progression_lightbox_video'): ?>
							
							href="<?php echo esc_url( get_post_meta($post->ID, 'progression_studios_external_link', true) );?>" data-rel="prettyPhoto[gallery-progression]" <?php $get_description = get_post(get_post_thumbnail_id())->post_excerpt; if(!empty($get_description)){ echo 'title="' . $get_description . '"'; } ?>
							<?php else: ?>
...
function progression_studios_post_count( $query ) {
	$portfolio_count = get_theme_mod('progression_studios_portfolio_posts_page', "12");
...
function progression_studios_show_pagination_links_pro()
{
    ...
    echo paginate_links( array(
            ...
            'total'     => $page_tot,
            'prev_next' => true,
				'prev_text'    => esc_html__('&lsaquo; Previous', 'zaser-progression'),
				'next_text'    => esc_html__('Next &rsaquo;', 'zaser-progression'),
            'end_size'  => 1,
            'mid_size'  => 2,
            'type'      => 'list'
        )
...
function progression_studios_infinite_content_nav_pro( $html_id ) {
	...
	if ( $wp_query->max_num_pages > 1 ) : ?>
		<div id="infinite-nav-pro">
			<div class="nav-previous"><?php next_posts_link( wp_kses( __('Load More <span><i class="fa fa-arrow-circle-down"></i></span>', 'zaser-progression' ) , TRUE) ); ?></div>
		</div>
	<?php endif;
}
endif;
